...
'''
# class Custom_Loss:
#     def _dice_coeff_loss(self, inputs: torch.Tenser, target: torch.Tensor) -> torch.Tensor:
#         smooth: float = 1.0
...
class Custom_Loss(nn.Module):
    ...
    def forward(self) -> torch.Tensor:
        return self._bce_dice_loss()
    


    def _dice_coeff_loss(self) -> torch.Tensor:
        smooth: float = 1.0
        intersection = 2.0 * ((self.target * self.inputs).sum()) + smooth
        union = self.target.sum() + self.inputs.sum() + smooth
        return 1 - (intersection / union)



    def _bce_dice_loss(self) -> torch.Tensor:
        dice_score = self._dice_coeff_loss()
        bce_score = nn.BCELoss()
        bce_loss = bce_score(self.inputs, self.target)
        return bce_loss + dice_score
    ...
    def _compute_IOU(self) -> float:
        ...
        with torch.no_grad():
            for i_step, (data, target) in enumerate(self.test_loader):
                ...
                pic_loss = self.metric(out_cut, target.data.cpu().numpy()).forward()
    ...
    def train_validate(self) -> _text:
        ...
        for epoch in range(self.num_epochs):
            ...
            for i_step, (data, target) in tqdm(enumerate(self.train_loader)):
                ...
                loss = self.criterion(outputs, target).forward()#_bce_dice_loss(outputs, target)
                #print(loss)
                ...
                print(f'Epoch : {epoch}')
                print(f'Mean Loss on Train : {np.array(losses).mean()}, Mean DICE on Train : {np.array(train_iou).mean()}, Mean DICE on Test : {val_mean_iou}')
...
if __name__.__contains__('__main__'):
    ...
    unet_model = UNet(1).to(device)
    # output = unet_model(torch.rand(1, 3, 256, 256).to(device))
    # print(output.shape)

    # ghmte ka pyramid model
    pyramid_model = Feature_Pyramid_Network().to(device)
    output = pyramid_model(torch.rand(1, 3, 256, 256).to(device))
    print(output.shape)
    
    #unet_encoder_decoder = ResNextUNet(1).to(device)
    #output = unet_encoder_decoder(torch.rand(1, 3, 256, 256).to(device))
    #print(output.shape)

    # print(unet_model)
    # print(pyramid_model)
    # print(unet_encoder_decoder)

    # unet_optimizer = torch.optim.Adamax(unet_model.parameters(), lr= 1e-3)
    # fpn_optimizer = torch.optim.Adamax(pyramid_model.parameters(), lr= 1e-3)
    # rx50_optimizer = torch.optim.Adam(unet_encoder_decoder.parameters(), lr= 5e-4)

    # num_epochs: int = 2

    # criterion = Custom_Loss
    # metric = dice_coef_metric

    # unet_Segmentor = Model(
    #         model_name= 'unet_encoder_decoder', 
    #         net= unet_encoder_decoder, 
